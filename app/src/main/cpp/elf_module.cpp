...
ElfW(Addr) elf_module::caculate_bias_addr(const ElfW(Ehdr) *elf) {
    ...
    const ElfW(Phdr) *phdr_table = reinterpret_cast<const ElfW(Phdr) *>(
            reinterpret_cast<uintptr_t>(elf) + offset);
...
bool elf_module::get_segment_view(void) {
    ...
    for (int i = 0; i < (int) this->m_dyn_size; i += 1, dyn += 1) {
        ...
        switch (dyn->d_tag) {
            case DT_SYMTAB:
                this->m_sym_ptr = reinterpret_cast<ElfW(Sym) *>(this->get_bias_addr() +
                                                                dyn->d_un.d_ptr);
                break;
            case DT_STRTAB:
                this->m_symstr_ptr = reinterpret_cast<const char *>(this->get_bias_addr() +
                                                                    dyn->d_un.d_ptr);
                break;
            case DT_PLTREL:
            ...
            case DT_REL:
            case DT_ANDROID_REL:
                this->m_reldyn_addr = reinterpret_cast<ElfW(Addr)>(this->get_bias_addr() +
                                                                   dyn->d_un.d_ptr);
                break;
            case DT_RELSZ:
            case DT_ANDROID_RELSZ:
                ...
                break;
            case DT_JMPREL:
                this->m_relplt_addr = reinterpret_cast<ElfW(Addr)>(this->get_bias_addr() +
                                                                   dyn->d_un.d_ptr);
                break;
            case DT_PLTRELSZ:
                ...
                break;
            case DT_HASH: {
                uint32_t *rawdata = reinterpret_cast<uint32_t *>(this->get_bias_addr() +
                                                                 dyn->d_un.d_ptr);
                ...
                log_info("nbucket: %d, nchain: %d, bucket: %p, chain:%p\n", this->m_nbucket,
                         this->m_nchain, this->m_bucket, this->m_chain);
                break;
            }
            case DT_GNU_HASH: {
                uint32_t *rawdata = reinterpret_cast<uint32_t *>(this->get_bias_addr() +
                ...
                this->m_gnu_bloom_filter = reinterpret_cast<ElfW(Addr) *>(this->get_bias_addr() +
                                                                          dyn->d_un.d_ptr + 16);
                this->m_gnu_bucket = reinterpret_cast<uint32_t *>(this->m_gnu_bloom_filter +
                                                                  this->m_gnu_maskwords);
...
template<class T>
void
elf_module::get_section_info(const char *name, ElfW(Shdr) **ppShdr, ElfW(Word) *pSize, T *data) {
...
template<class T>
void elf_module::get_segment_info(const ElfW(Word) type, ElfW(Phdr) **ppPhdr, ElfW(Word) *pSize,
                                  T *data) {
    ...
    if (this->get_is_gnu_hash()) {
        ...
        if (!result) {
            for (int i = 0; i < (int) this->m_gnu_symndx; i++) {
                char const *symName = reinterpret_cast<char const *>(this->m_sym_ptr[i].st_name +
                                                                     this->m_symstr_ptr);
...
bool elf_module::hook(const char *symbol, void *replace_func, void **old_func) {
    ...
    int relplt_counts = this->get_is_use_rela() ? this->m_relplt_bytes / sizeof(ElfW(Rela)) :
                        this->m_relplt_bytes / sizeof(ElfW(Rel));
    for (uint32_t i = 0; i < relplt_counts; i++) {
        ...
        if (this->get_is_use_rela()) {
            ElfW(Rela) *rela = reinterpret_cast<ElfW(Rela) *>(this->m_relplt_addr +
                                                              sizeof(ElfW(Rela)) * i);
            r_info = (unsigned long) rela->r_info;
            r_offset = rela->r_offset;
        } else {
            ElfW(Rel) *rel = reinterpret_cast<ElfW(Rel) *>(this->m_relplt_addr +
                                                           sizeof(ElfW(Rel)) * i);
    ...
    int reldyn_counts = this->get_is_use_rela() ? this->m_reldyn_bytes / sizeof(ElfW(Rela)) :
                        this->m_reldyn_bytes / sizeof(ElfW(Rel));
    for (uint32_t i = 0; i < reldyn_counts; i++) {
        unsigned long r_info = 0;   // for Elf32 it's Elf32_Word, but Elf64 it's Elf64_Xword.
        ElfW(Addr) r_offset = 0;
        if (this->get_is_use_rela()) {
            ElfW(Rela) *rela = reinterpret_cast<ElfW(Rela) *>(this->m_reldyn_addr +
                                                              sizeof(ElfW(Rela)) * i);
            r_info = (unsigned long) rela->r_info;
            r_offset = rela->r_offset;
        } else {
            ElfW(Rel) *rel = reinterpret_cast<ElfW(Rel) *>(this->m_reldyn_addr +
                                                           sizeof(ElfW(Rel)) * i);
            r_info = (unsigned long) rel->r_info;
            r_offset = rel->r_offset;
        }
...
void elf_module::dump_sections(void) {
    ...
    for (int i = 0; i < shnum; i += 1, shdr += 1) {
        const char *name = shdr->sh_name == 0 || !this->m_shstr_ptr ? "UNKOWN" : (const char *) (
                shdr->sh_name + this->m_shstr_ptr);
...
void elf_module::dump_rel_info(void) {
    ...
    ElfW(Word) resszs[] = {ElfW(Word)(this->m_reldyn_bytes / sizeof(ElfW(Rel))),
                           ElfW(Word)(this->m_relplt_bytes / sizeof(ElfW(Rel)))};
...
void elf_module::dump_rela_info(void) {
    ...
    ElfW(Word) resszs[] = {ElfW(Word)(this->m_reldyn_bytes / sizeof(ElfW(Rela))),
                           ElfW(Word)(this->m_relplt_bytes / sizeof(ElfW(Rela)))};
